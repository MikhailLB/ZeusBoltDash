...
/// `flutter.zbd_gate_cold_url`. The `flutter.` prefix lets SharedPreferences
/// read it via ZeusLinkBridge.consumeTapUrl() with no MethodChannel needed.
class SceneDelegate: FlutterSceneDelegate {
  static let coldUrlKey = "flutter.zbd_gate_cold_url"
  ...
  static func extractUrl(from userInfo: [AnyHashable: Any]) -> String? {
    ...
    #if DEBUG
    NSLog("[ZBD.NATIVE] userInfo keys: %@", userInfo.keys.map { "\($0)" }.joined(separator: ", "))
    for (k, v) in userInfo { NSLog("[ZBD.NATIVE] userInfo[\(k)] = \(v)") }
    #endif
    ...
    NSLog("[ZBD.NATIVE] no url found in userInfo")
    return nil
  }

  static func persist(url: String) {
    NSLog("[ZBD.NATIVE] cold-start url -> %@", url)
