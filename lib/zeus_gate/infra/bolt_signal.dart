...
class BoltSignal {
  ...
  Future<void> _doWarmup() async {
    ...
    try {
      ...
      final opts = AppsFlyerOptions(
        afDevKey: devKey,
        appId: BoltConfig.analyticsAppId,
  ...
  Future<Map<String, dynamic>?> _refreshGcd() async {
    try {
      ...
      final appId = Platform.isIOS ? BoltConfig.analyticsAppId : BoltConfig.bundleId;
      final url = gcdUrl(appId, uid);
      if (url.isEmpty) return null;
      final resp = await zeusAgent.get(
        Uri.parse(url),
        headers: {'authorization': 'Bearer ${BoltConfig.installKey}'},
  ...
  Future<Map<String, dynamic>> buildPayload({
    ...
    body['bundle_id'] = BoltConfig.bundleId;
    body['store_id']  = BoltConfig.platformStoreId;
    body['os']        = Platform.isAndroid ? 'Android' : 'iOS';
    body['locale']    = locale;
    if (pushToken != null && pushToken.isNotEmpty) body['push_token'] = pushToken;
    if (BoltConfig.firebaseNumber.isNotEmpty) {
