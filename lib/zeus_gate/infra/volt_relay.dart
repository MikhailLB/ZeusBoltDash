...
@pragma('vm:entry-point')
Future<void> voltTrayTapHandler(NotificationResponse resp) async {
  final payload = resp.payload;
  if (payload == null || payload.isEmpty) return;
  try {
    final d = jsonDecode(payload);
    if (d is Map && d['url'] is String && (d['url'] as String).isNotEmpty) {
      await FlashVault().stashOneShotUrl(d['url'] as String);
    }
  } catch (_) {}
}
...
class VoltRelay {
  ...
  Future<void> _setupTray() async {
    await _tray.initialize(
      ...
      onDidReceiveBackgroundNotificationResponse: voltTrayTapHandler,
    );
    if (Platform.isAndroid) {
      ...
      await impl?.createNotificationChannel(const AndroidNotificationChannel(
        ...
        description: 'Zeus Bolt Dash real-time alerts',
  ...
  Future<Uint8List?> _fetchImage(String url) async {
    try {
      final r = await zeusAgent.get(Uri.parse(url))
